...
const SalesStatusBadge = ({ sale }) => {
    ...
    if (sale.status === 'CANCELLED') {
        ...
        switch (sale.paymentStatus) {
            ...
            case 'DUE': config = { text: 'Due', className: 'bg-red-100 text-red-800' }; break;
            default: break;
        }
    }
...
const SaleDetailHeader = ({ sale, onPrint, onCancelClick, onPaymentClick, canCancel, canRecordPayment }) => (
    <div className="flex flex-col items-start justify-between gap-4 pb-6 border-b print:hidden md:flex-row md:items-center">
        <div>
            <div className="flex items-center gap-4">
                <h1 className="text-3xl font-bold tracking-tight text-gray-900">{sale.invoiceNumber}</h1>
                <SalesStatusBadge sale={sale} />
            </div>
            <p className="mt-1 text-sm text-gray-500">
                Sale recorded on {new Date(sale.createdAt).toLocaleString()}
            </p>
        </div>
        <div className="flex items-center flex-shrink-0 gap-2">
            <button onClick={onPrint} className="inline-flex items-center gap-2 px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50">
                <Printer size={16} /> Print
            </button>
            {canRecordPayment && (
                <button onClick={onPaymentClick} className="inline-flex items-center gap-2 px-3 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-md shadow-sm hover:bg-green-700">
                    <Banknote size={16} /> Record Payment
                </button>
            )}
            {canCancel && (
                <button onClick={onCancelClick} className="inline-flex items-center gap-2 px-3 py-2 text-sm font-medium text-white bg-red-600 border border-transparent rounded-md shadow-sm hover:bg-red-700">
    ...
    <div>
        ...
        <address className="mt-2 text-sm not-italic text-gray-600">
            {customer ? (
                ...
                <p className="font-medium text-gray-900">Cash Sale / Walk-in Customer</p>
            )}
        </address>
    </div>
);

const SaleMetadata = ({ sale }) => (
     <div className="text-sm">
        <dl className="grid grid-cols-2 gap-x-4 gap-y-2">
            <dt className="font-semibold text-gray-800">Sale Date:</dt>
            <dd className="text-gray-600">{new Date(sale.saleDate).toLocaleDateString()}</dd>
            <dt className="font-semibold text-gray-800">Sale Type:</dt>
            <dd className="text-gray-600">{sale.saleType}</dd>
            <dt className="font-semibold text-gray-800">Created By:</dt>
            <dd className="text-gray-600">{sale.createdBy ? `${sale.createdBy.fname} ${sale.createdBy.lname}` : 'N/A'}</dd>
        </dl>
    </div>
);
                ...
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-slate-50">
                        <tr>
                            ...
                            <th className="px-4 py-2 text-xs font-semibold tracking-wider text-right text-gray-600 uppercase">Price at Sale</th>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {items.map((item, index) => (
                            <tr key={index}>
                                ...
                                <td className="px-4 py-3 text-sm text-right text-gray-600">{item.priceAtSale.toLocaleString('en-IN', { style: 'currency', currency: 'NPR' })}</td>
...
const RecordPaymentModal = ({ isOpen, onClose, sale }) => {
    const { mutate: recordPayment, isPending } = useRecordPaymentForSale();
    const PaymentSchema = Yup.object().shape({
        amountPaid: Yup.number()
            .positive('Amount must be positive.')
            .max(sale.amountDue, `Payment cannot exceed the amount due of Rs. ${sale.amountDue.toFixed(2)}.`)
                    ...
                    <div className="flex items-center justify-center min-h-full p-4 text-center">
                        <Transition.Child as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
                            <Dialog.Panel className="w-full max-w-md mx-auto overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
                                <Formik
                                    initialValues={{ amountPaid: sale.amountDue, paymentMethod: 'CASH' }}
                                    validationSchema={PaymentSchema}
                                    onSubmit={(values, { setSubmitting }) => {
                                        recordPayment({ id: sale._id, ...values }, { onSuccess: () => { setSubmitting(false); onClose(); } });
                                    }}
                                >
                                    {({ isSubmitting }) => (
                                        <Form>
...
const SaleDetailsPage = () => {
    ...
    const { data: sale, isLoading, isError, error } = useGetSaleById(id);
    ...
    if (isLoading) {
        return <div className="flex items-center justify-center h-screen"><Loader2 className="w-12 h-12 text-orange-500 animate-spin" /></div>;
    }
    ...
    if (isError || !sale) {
        return (
            <div className="flex flex-col items-center justify-center h-screen p-8 text-center text-red-600 bg-red-50" role="alert">
                ...
                <p>{error?.message || 'The sale could not be found or an error occurred.'}</p>
                <Link to="/sales" className="inline-block px-4 py-2 mt-4 text-white bg-blue-500 rounded-md hover:bg-blue-600">Back to Sales</Link>
            </div>
        );
    }
        ...
        <div className="min-h-screen p-4 bg-slate-50 sm:p-6 lg:p-8">
            <div className="max-w-4xl mx-auto">
                ...
                <div className="p-8 bg-white shadow-lg print:shadow-none rounded-xl ring-1 ring-gray-900/5 print:ring-0">
                    ...
                    <SaleFinancials sale={sale} />
                    
                    {sale.notes && (
                        <div className="pt-6 mt-6 border-t">
                            <h3 className="text-sm font-semibold text-gray-800">Notes</h3>
                            <p className="mt-1 text-sm text-gray-600 whitespace-pre-wrap">{sale.notes}</p>
                        </div>
                    )}
                </div>
            </div>
            ...
            {canCancel && (
                <ConfirmationModal
                    ...
                    title="Cancel Sale"
                    message={`Are you sure you want to cancel Invoice #${sale.invoiceNumber}? This will restock items and reverse any transactions. This action cannot be undone.`}
